...
console.log(state)
  ...
  console.log(token,'tkn',initialState)
  

  Axios.post('http://127.0.0.1:8000/api/OrderData/',{
    DesignName:"2D Bed Design",
    CustomisedDesign:JSON.parse(localStorage.getItem(APP_NAMESPACE))
...
export const delete1 = () => {
  ...
  Axios.post('http://127.0.0.1:8000/api/deleteElementData/',{
    jsonArray:initialState,
    elementId:state.selected
  }).then((res)=>{
    console.log(res.data.jsonArray)
    localStorage.setItem(APP_NAMESPACE, JSON.stringify(res.data.jsonArray));
